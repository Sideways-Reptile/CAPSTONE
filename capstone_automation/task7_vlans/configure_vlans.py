...
Trunk topology (HQ):
  pfSense em1 ←→ SW1-Core (port 1) ←→ SW2-Dist (port 1-2)
  SW2-Dist ←→ SW3-Access-1 (port 1)
  SW2-Dist ←→ SW4-Access-2 (port 1)
  SW1-Core ←→ SW5-Access-3 (port 1)

Usage:
  python3 configure_vlans.py              # configure all switches
  python3 configure_vlans.py --verify     # verify VLANs only
  python3 configure_vlans.py --host 10.10.10.21
"""
...
SWITCH_PORT_CONFIG = {
    "HQ-SW1-Core": {
        ...
        # Ports 2-3 = downlinks to SW2-Dist (redundant trunk)
        # Port 4 = downlink to SW5-Access-3
        "trunk_ports":  [1, 2, 3, 4],
    ...
    "HQ-SW2-Dist": {
        # Port 1-2 = uplinks to SW1-Core (redundant)
        # Ports 3-5 = downlinks to Access switches
        "trunk_ports":  [1, 2, 3, 4, 5],
        "access_ports": {},
        "mgmt_ip":      "10.10.10.22",
        "mgmt_vlan":    10,
    },
    "HQ-SW3-Access-1": {
        # Port 1 = uplink to SW2-Dist
        # Port 24 = access port for CORP endpoint
        "trunk_ports":  [1],
        "access_ports": {24: 20},   # port 24 = CORP_NET (VLAN 20)
        "mgmt_ip":      "10.10.10.23",
        "mgmt_vlan":    10,
    },
    "HQ-SW4-Access-2": {
        ...
        "trunk_ports":  [1],
        "access_ports": {24: 30},   # port 24 = DMZ_NET (VLAN 30)
        "mgmt_ip":      "10.10.10.24",
        "mgmt_vlan":    10,
    },
    "HQ-SW5-Access-3": {
        ...
        "trunk_ports":  [1],
        "access_ports": {24: 40},   # port 24 = GUEST_NET (VLAN 40)
        "mgmt_ip":      "10.10.10.25",
        "mgmt_vlan":    10,
    },
    "BR-SW1": {
